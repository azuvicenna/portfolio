...
import { ref } from 'vue';
import {
  Plus, Pencil, Trash2, X, GraduationCap, Calendar, Loader2, AlertTriangle
...
const openCreateModal = () => {
  ...
  form.value = { id: null, degree: '', institution: '', score: '', startDate: '', endDate: '', description: '' };
...
const saveEducation = () => {
  ...
  setTimeout(() => {
    if (isEditing.value) {
      ...
      if (index !== -1) educations.value[index] = { ...form.value };
    } else {
      educations.value.unshift({ ...form.value, id: Date.now() });
    }
...
const deleteEducation = () => {
  ...
  isSubmitting.value = true;
  setTimeout(() => {
    educations.value = educations.value.filter(e => e.id !== itemToDelete.value.id);
    isSubmitting.value = false;
    closeDeleteModal();
  }, 800);
};
...
<template>
  <div class="max-w-6xl mx-auto space-y-8">
    ...
    <div class="bg-white rounded-[2rem] shadow-[0_2px_20px_rgb(0,0,0,0.04)] border border-gray-100 overflow-hidden">
      <div v-if="educations.length === 0" class="p-12 text-center text-slate-400">
        <GraduationCap :size="48" class="mb-4 opacity-50 mx-auto" />
        ...
        <tbody class="divide-y divide-gray-50">
          <tr v-for="item in educations" :key="item.id" class="group hover:bg-gray-50/80 transition duration-200">
            ...
            <td class="px-6 py-5 align-top">
              <div
                class="flex items-center gap-2 text-sm text-slate-600 font-medium bg-emerald-50 px-3 py-1.5 rounded-lg w-fit text-emerald-700 border border-emerald-100">
                <span class="font-bold">{{ item.score.split('/')[0] }}</span> / {{ item.score.split('/')[1] || '?' }}
              </div>
            </td>
            <td class="px-6 py-5 align-top">
              <div class="flex items-center gap-2 text-sm">
                ...
                <span class="font-medium text-slate-700">
                  ...
                  <span :class="!item.endDate ? 'text-indigo-600 font-bold' : ''">{{ formatDate(item.endDate) }}</span>
                </span>
              </div>
            </td>
              ...
              <form @submit.prevent="saveEducation" class="space-y-6">
                ...
                <div class="flex justify-end gap-3 pt-6 border-t border-gray-100">
                  ...
                  <button type="submit" :disabled="isSubmitting"
                    ...
                    <Loader2 v-if="isSubmitting" class="animate-spin" :size="20" /><span v-else>Save Education</span>
                  </button>
                </div>
              </form>
            </div>
        ...
        <div v-if="isDeleteModalOpen" class="fixed inset-0 z-[1000] flex items-center justify-center p-4">
          ...
          <div
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              <button @click="deleteEducation" :disabled="isSubmitting"
